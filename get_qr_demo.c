...
uint8_t writePNG( QRcode *qrcode, char *outfile)
{
...
uint8_t encode(char* qr_string,char* png_file_path)
{
	uint8_t b;
	QRcode *qrcode=QRcode_encodeString(qr_string,0,QR_ECLEVEL_H,QR_MODE_8,1);
    	if(!qrcode)
        	return false;
...
{    
	//encode("https://blog.rangnicaidaosi.top","./test.png");
	if(argc<3){
		printf("./%s qr_info_string file_path_save_sting\r\n",argv[0]);
		return 1;
	}else{
